import { selectTradingLastErrorMessageByTradeType, tradingBuyActions } from '@suite-common/trading';
...
describe('LastErrorMessage', () => {
    ...
    it('should clear error message on unmount', () => {
        const errorMessage = 'An error occurred during the buy process';
        store.dispatch(tradingBuyActions.setLastErrorMessage(errorMessage));
        const { unmount } = renderLastErrorMessage({ tradingType: 'buy' });

        unmount();

        expect(selectTradingLastErrorMessageByTradeType(store.getState(), 'buy')).toBeUndefined();
    });
});
