...
test.describe('Global receive and send', { tag: ['@T3T1', '@T3W1'] }, () => {
  ...
  test(`Global receive`, async ({
    ...
    await test.step('Generate and compare addresses', async () => {
      ...
      ).toHaveTranslation('LABELING_ACCOUNT', { values: { networkName: 'Ethereum', index: '3' } });
      await walletPage.revealAddressButton.click();
      const addressDisplayedInSuite = await devicePrompt.outputValueOf('address').textContent();
      ...
      const addressDisplayedOnDevice = await devicePrompt.getAddress();
  ...
  test(`Global send`, async ({ page, devicePrompt }) => {
    ...
    await test.step('Send form validation', async () => {
      ...
      ).toHaveTranslation('LABELING_ACCOUNT', { values: { networkName: 'Bitcoin', index: '3' } });
    })
  });
});
