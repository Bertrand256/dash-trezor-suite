...
class BluetoothManager {
            ...
            if (scannedDevice) {
                ...
                if (nearbyDeviceIndex >= 0) {
                    ...
                    this.nearbyDevices.unshift({
                        ...
                        data: Array.from(
                            Buffer.from(scannedDevice.manufacturerData ?? '', 'base64'),
                        ).slice(2), // strip company identifier
                        lastUpdatedTimestamp: Date.now(),
    ...
    private handleConnectionError(deviceId: string, error: any) {
        ...
        if (error.iosErrorCode === 14 || error.reason === 'Peer removed pairing information') {
            this.updateDeviceConnectionStatusChange({
                deviceId,
